...
<html lang="en">
	...
	<body>
		<div class="sidebar close">
		  ...
		  <section class="home-section" style="height: 200vh;">
			<div class="home-content">
			  ...
			  <span class="text">Train Service Maintenance System</span>
			</div>
                        ...
                        while ($serviceRequest = $serviceRequestStmt->fetch(PDO::FETCH_ASSOC)) {
                            ...
                            <tr>
                                ...
                                <button class="status-btn <?php echo $statusClass; ?>"
                                       ...
                                       onclick="toggleProgress(<?php echo $serviceRequest['request_id']; ?>, <?php echo isset($serviceRequest['status']) ? $serviceRequest['status'] : 0; ?>)">
		...
		// Function to toggle the service status
function toggleProgress(serviceRequestId, currentStatus) {
    if (serviceRequestId) {
        // confirm action (optional, but good for UX)
        // var confirmation = confirm("Do you want to change the status?");
        ...
        // Note: We send the CURRENT status. The PHP logic you wrote flips it (1 becomes 0, 0 becomes 1).
        xhr.send("serviceRequestId=" + serviceRequestId + "&status=" + currentStatus + "&action=toggleProgress");

        xhr.onreadystatechange = function() {
            if (xhr.readyState == 4 && xhr.status == 200) {
                // Reload the page to see the button color change
                location.reload();
            }
        };
    } else {
