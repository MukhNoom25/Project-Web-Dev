...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["updateCategory"])) {
    ...
    $updateQuery = "UPDATE categories SET category = :category, status = :status WHERE id = :id";
    ...
    if ($updateStmt->execute()) {
        // Display success message
        echo '<script>alert("Train category updated successfully!");</script>';

        // Redirect to category.php after 500 milliseconds
        echo '<script>window.location.href = "category.php";</script>';
        exit();
    } else {
        // Handle the case where update failed
        echo "Error: Unable to update train category.";
    }
}
...
if (isset($_GET['id'])) {
    ...
    // Fetch the existing category data from the database
    ...
    if (!$category) {
        // Redirect to the category.php page if the category is not found
        header("Location: category.php");
        exit();
    }
} else {
    // Redirect to the category.php page if the ID is not set
    header("Location: category.php");
    exit();
}
...
<html lang="en">
	<head>
		...
		<title>Update Train Category</title>
		  ...
		  <section class="home-section">
			...
			<div class="content">
				<div class="content-header">
					<h3 class="content-title">Update Train Category</h3>
				</div>
                ...
                <div class="update-category-form">
                    <form action="" method="post">
                        ...
                        <div class="form-buttons">
                            <button type="submit" class="btn btn-flat btn-primary" name="updateCategory">Update</button>
