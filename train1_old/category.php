...
if ($statusFilter == 'all') {
    ...
    $categoryQuery = "SELECT * FROM categories WHERE status = 1";
} elseif ($statusFilter == 'inactive') {
    $categoryQuery = "SELECT * FROM categories WHERE status = 0";
} else {
	$categoryQuery = "SELECT * FROM categories WHERE status = 1 OR status = 0";
}
...
if ($statusFilter != 'all' && $statusFilter != 'active' && $statusFilter != 'inactive') {
    ...
    $categoryStmt->bindParam(':status', $statusFilter, PDO::PARAM_INT);
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['action'])) {
        if ($_POST['action'] == 'toggleStatus' && isset($_POST['categoryId']) && isset($_POST['status'])) {
            $categoryId = $_POST['categoryId'];
            $newStatus = $_POST['status'] == 1 ? 0 : 1;

            // Update the status in the database
            $updateStatusQuery = "UPDATE categories SET status = :status WHERE id = :id";
            $updateStatusStmt = $pdo->prepare($updateStatusQuery);
            $updateStatusStmt->bindParam(':status', $newStatus);
            $updateStatusStmt->bindParam(':id', $categoryId);
	...
	if (isset($_POST['categoryId']) && isset($_POST['action']) && $_POST['action'] == 'delete') {
		$categoryId = $_POST['categoryId'];
...
<html lang="en">
	<head>
		...
		<title>Train Category Management</title>
	...
	<body>
		<div class="sidebar close">
                    ...
                    <table class="categories-table">
						...
						<?php
							...
							while ($category = $categoryStmt->fetch(PDO::FETCH_ASSOC)) {
								...
								$statusClass = ($category['status'] == 1) ? 'active' : 'inactive';
                                ...
                                <tr>
                                    ...
                                    <td>
										<button class="status-btn <?php echo $statusClass; ?>" data-category-id="<?php echo $category['id']; ?>" onclick="toggleStatus(<?php echo $category['id']; ?>, <?php echo $category['status']; ?>)">
											<?php echo $category['status'] == 1 ? 'Active' : 'Inactive'; ?>
										</button>
									</td>
									<td><?php echo htmlspecialchars($category['date_created']); ?></td>
									<td class="actions-column">
										<button class="update-btn" data-category-id="<?php echo $category['id']; ?>">Update</button>
										<button class="delete-btn" data-category-id="<?php echo $category['id']; ?>" onclick="confirmDelete(<?php echo $category['id']; ?>)">Delete</button>
									</td>
								</tr>
								<?php
							}
		  ...
		  <script defer>
			...
			var updateCategoryButtons = document.querySelectorAll(".update-btn");

			// Add a click event listener to each "update-btn" element for categories
			updateCategoryButtons.forEach(function(button) {
				button.addEventListener("click", function(event) {
					...
					window.location.href = "update_category.php?id=" + categoryId;
				});
			});
		...
		function toggleStatus(categoryId, currentStatus) {
			...
			xhr.send("categoryId=" + categoryId + "&status=" + currentStatus + "&action=toggleStatus");
		...
		function confirmDelete(categoryId) {
			...
			if (confirmation) {
				...
				xhr.send("categoryId=" + categoryId + "&action=delete");
