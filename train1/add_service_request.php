...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["saveServiceRequest"])) {
    ...
    $insertQuery = "INSERT INTO service_request (train_type_id, train_brand, train_registration_number, train_model, gate_number, service_id, assigned_to_id, status, user_id) VALUES (:trainTypeId, :trainBrand, :trainRegistrationNumber, :trainModel, :gateNumber, :serviceId, :assignedToId, :status, :createdById)";
			...
			<div class="content">
                ...
                <div class="add-services-request-form">
                    <form action="" method="post">
						...
						<!-- Replace the gate number input with this dropdown -->
						<div class="form-group">
							...
							<select id="gateNumber" name="gateNumber" required>
								...
								$gatesStmt->execute();
								while ($gate = $gatesStmt->fetch(PDO::FETCH_ASSOC)) {
									echo '<option value="' . htmlspecialchars($gate['gate_name']) . '">' . htmlspecialchars($gate['gate_name']) . '</option>';
								}
								?>
							</select>
						</div>
